...
def inverse_lerp(a, b, t):
  """ linear interpolation function """
  return a + t * (b - a)

def generate_noise_map(row, col, depth, octaves, persistence, lacunarity):
  """ 
    function to generate the noise map using perlin noise.
    values of the noise map skew larger towards the center
  """
  noise_map = []
  ...
  for r in range(0, row):
    ...
    for c in range(0, col):
      ...
      for octave in range(0, octaves):
        # apply sin function to x and y values so values near center are larger
        col = (depth * math.sin((math.pi/(2 * col))*c))
        row = (depth * math.sin((math.pi/(2 * row))*r))
        
        # apply perlin noise function to randomly alter height vaalue
        perlin = mathutils.noise.noise(mathutils.Vector(col, row), 'PERLIN_ORIGINAL')
        noise_height += perlin * amplitude

        amplitude *= persistence
        frequency *= lacunarity 

      # store the min and max noises
      if max_noise == math.nan or noise_height > max_noise:
        max_noise = noise_height
      if min_noise == math.nan or noise_height < min_noise:
        min_noise = noise_height
      
      # set the value of a coordinate to height
      noise_map_row[c] = noise_height
    noise_map[r] = noise_map_row

  # normalize noise_map
  for r in range(0, row):
    for c in range(0, col):
      noise_map[r][c] = inverse_lerp(min_noise, max_noise, noise_map[r][c]) * -1
